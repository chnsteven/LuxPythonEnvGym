...
class AgentPolicy(AgentWithModel):
    ...
    def get_observation(self, game, unit, city_tile, team, is_new_turn):
        ...
        # 归一化常数（从游戏常量推导，避免魔法数字）
        # 曼哈顿距离最大值：对角线两端 (0,0) → (W-1, H-1)
        MAX_DISTANCE = (game.map.width - 1) + (game.map.height - 1)  # 最大 62（32×32地图）
        # 城市 tile / 单位数量上限：整张地图面积（每个 city tile 最多产出一个单位）
        MAX_CITY_TILES = game.map.width * game.map.height             # 最大 1024
        MAX_UNITS      = game.map.width * game.map.height             # 最大 1024
        ...
        if pos is not None:
            ...
            for i in range(3):
                if i < len(resources):
                    ...
                    obs[idx + 3] = r['distance'] / MAX_DISTANCE
                idx += 4
        else:
            idx += 12
        ...
        if pos is not None:
            ...
            for i in range(3):
                if i < len(friendly_units):
                    ...
                    obs[idx + 2] = u['distance'] / MAX_DISTANCE
                    # cargo_left (1维): normalized by RESOURCE_CAPACITY.WORKER = 100
                    obs[idx + 3] = u['cargo_left'] / CARGO_CAPACITY
                idx += 4
        else:
            idx += 12
        ...
        # ═══════════════════════════════════════════════════════════════════════
        if pos is not None:
            # 计算当前夜晚剩余回合（用于 fuel_survival）
            _day_len   = GAME_CONSTANTS["PARAMETERS"]["DAY_LENGTH"]    # 30
            _night_len = GAME_CONSTANTS["PARAMETERS"]["NIGHT_LENGTH"]  # 10
            _cycle_len = _day_len + _night_len
            _cycle_pos = game.state["turn"] % _cycle_len
            # 白天：下一个夜晚是完整的 NIGHT_LENGTH；夜晚：当前夜晚剩余回合
            nights_left = _night_len if _cycle_pos < _day_len else (_cycle_len - _cycle_pos)
            nights_left = max(nights_left, 1)  # 避免除零
            ...
            for city in game.cities.values():
                if city.team == team:
                    ...
                    fuel_survival = min(city.fuel / (upkeep * nights_left), 1.0) if upkeep > 0 else 1.0
            ...
            for i in range(2):
                if i < len(friendly_cities):
                    c = friendly_cities[i]
                    # distance (1维): normalized by MAX_DISTANCE
                    obs[idx] = c['distance'] / MAX_DISTANCE
        ...
        if pos is not None:
            ...
            for i in range(2):
                if i < len(enemy_cities):
                    ...
                    obs[idx + 1] = c['tile_count'] / MAX_CITY_TILES
                idx += 2
        else:
            idx += 4
        ...
        obs[idx] = worker_count / MAX_UNITS
        obs[idx + 1] = cart_count / MAX_UNITS
        idx += 2
        ...
        obs[idx] = enemy_worker_count / MAX_UNITS
        obs[idx + 1] = enemy_cart_count / MAX_UNITS
        idx += 2
